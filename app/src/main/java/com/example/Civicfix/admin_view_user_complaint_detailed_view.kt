...
import com.example.Civicfix.databinding.ActivityAdminViewUserComplaintDetailedViewBinding
...
class admin_view_user_complaint_detailed_view : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        seeFeedbackButton.setOnClickListener {
            complaintUserRef.child("feedback").addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    val feedback = snapshot.getValue(String::class.java)
                    if (feedback != null && feedback.isNotEmpty()) {
                        Toast.makeText(this@admin_view_user_complaint_detailed_view, "Feedback: $feedback", Toast.LENGTH_LONG).show()
                    } else {
                        Toast.makeText(this@admin_view_user_complaint_detailed_view, "No feedback available", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@admin_view_user_complaint_detailed_view, "Error loading feedback: ${error.message}", Toast.LENGTH_SHORT).show()
                }
            })
        }
